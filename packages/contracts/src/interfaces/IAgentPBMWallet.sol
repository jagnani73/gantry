...
interface IAgentPBMWallet {
    ...
    ///         key's EIP-712 signature over (intentId, token, amount) plus every policy
    ///         dimension, then transfer `amount` of `token` to msg.sender (the core).
    ///         Reverts with a policy error otherwise.
