...
- **The demo payer is a single shared account.** When the deployed build is configured with `NEXT_PUBLIC_DEMO_KEY`, every visitor signs with the same key — so the balance, the activity feed and the agent list are shared, and the payment history is not private. The payer app says so on screen, and **Settings → "Use my own wallet instead"** switches the signer to a connected wallet, which is the private path. Nothing migrates across: an activity feed is the settlements whose on-chain payer is that address and an agent is a wallet that address owns, so both stay with the account they belong to.
