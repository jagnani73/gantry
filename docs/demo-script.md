...
| Agents screen is empty | For an owner the list is one `walletsOf(owner)` call, always current, so an empty list means the demo account owns no wallet yet. `pnpm demo:reset` provisions one |
...
| **The agent refuses with `agent_currency_mismatch`** | It was told to pay in a currency its wallet does not hold. One currency per agent, enforced at the door — `AGENT_PAY_TOKEN` in `packages/agent/.env` must match what the pinned wallet actually holds. "Kopi Runner" holds USDC and is what `demo-reset` provisions; "Euro Runner" holds EURC and is created by `demo-seed`. Pin the one you mean with `AGENT_WALLET`, and set `AGENT_PAY_TOKEN` to match what it holds |
