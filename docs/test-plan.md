...
**Two fixes awaiting your re-test**, both on the agents list, both found by
driving Phase 7, both contained (renaming an agent is in no demo beat):

- **F21 → re-test #50.** The list rendered a name the payer had just replaced as
  fact, beside the new caps. `settleAgentExpectation` now carries the summary
  that settled it, so the guard and the data can only be cleared together.
- **F4 → re-test #53.** The list then froze on `Saving…` forever, because the
  give-up rule lives with whoever polls and this screen does not poll. Fixed by
  removing the wait rather than bounding it: the row now renders what the mined
  receipt proves, marked `Just changed · reading it back`.

The two are opposite failures of one mechanism — showing stale data as fact, and
refusing to show anything at all — which is why the fix ended up being "render
what the receipt proves" rather than a better spinner.
...
| 53  | **F4 probe:** save from a form opened WITHOUT the detail screen behind it (`/app/agents?edit=<wallet>`, or the denial-remedy button), against a list read that cannot catch up | Does the row clear from "Saving…", or sit there with nothing explaining why?                                        | **CONFIRMED → F4**, fix landed, **MUST BE RE-RUN.** Froze on `Saving…` indefinitely; the only exit was opening the agent, which nothing names. Re-run: the row must now show the NEW name and caps immediately, with `Just changed · reading it back` under the address, and that line must disappear on its own once the read agrees |
