...
**FOUR RESULTS WENT STALE ON 21 AUG** — #50, #53, #65 and #73 — because the code
review that followed changed the behaviour they measured. **All four have been
re-run and pass.** A pass against superseded behaviour is not a pass, which is
the reason they were re-opened rather than left green. Not because
they were wrong when recorded, but because the code review that followed
changed the behaviour they measured: the freshness overlay gained the two
fields a policy receipt actually proves and a 60s expiry, the agent's gateway
errors are reported rather than silenced, and the merchant-write limits gained
a per-handle share plus different accounting on unknown outcomes. A pass
against superseded behaviour is not a pass. See each row.
...
**F27–F37 came from a SECOND four-agent review**, run over the 15 commits of this
sprint's own fixes on 21 Aug (general code, silent failures, comment accuracy,
type design). Eleven findings, all fixed across eight commits. The pattern worth
keeping: four of them were regressions introduced BY the fixes in this same
file — a projection that omitted the two fields a policy receipt actually
proves, a guard the overlay made unsatisfiable, an expectation with no expiry,
and accounting that charged only successes. Reviewing your own fixes is not
optional; the first pass produced the bugs the second pass found.

One claim was WITHDRAWN rather than fixed: the "25 requests from 25 forwarded
IPs" measurement cited in code, CLAUDE.md and this file was a localhost
artefact — `trust proxy = 1` means a real proxy appends the client address, so
a forged header cannot move `req.ip` on Render. The conclusion it supported
still holds for genuinely distinct addresses; the evidence did not.
...
| 50  | While that read catches up                                                                                                                                     | List row shows **"Saving…"**, never the new categories beside the old name                                          | **RE-RUN PASSES (21 Aug).** Driven against a counter carrying a real S$4.50 from the #65 run, which is the only state that can tell the fix from the bug: lowering the daily cap to S$4.00 now reads **S$0.00 of S$4.00 today**, where the overlay used to render S$4.50 of S$4.00 — a spend above its own cap, a state no block has ever held. The form's new reset notice rendered with that same figure in it. Demo agent restored afterwards (S$50.00/day, food_beverage, spent S$0.00) |
| 51  | Reject the **second** wallet prompt                                                                                                                            | Partial-save toast names specifically which write landed                                                            | **PASS** (owner-reported) — rejecting the second prompt led with what had already mined before the wallet's own rejection message |
| 52  | Revoke                                                                                                                                                         | Chip flips to Revoked with no stale "Active"                                                                        | **PASS** (owner-reported) — chip flipped to Revoked with no stale Active |
| 53  | **F4 probe:** save from a form opened WITHOUT the detail screen behind it (`/app/agents?edit=<wallet>`, or the denial-remedy button), against a list read that cannot catch up | Does the row clear from "Saving…", or sit there with nothing explaining why?                                        | **RE-RUN PASSES (21 Aug).** Overlay shows the new values with `Just changed · reading it back`, and past the 60s TTL the row falls back to the doctored read on its own — the receipt no longer outranks a read forever |
...
| 65  | `AISA_MODEL` pointed at a paid id                                   | Fails at the first tool call with `recharge_required`, not at startup  | **RE-RUN PASSES (21 Aug).** One line, `[agent] 402 model gemini-3.5-flash requires a paid balance…`, then scripted beats, settlement `0x73e3d632…`, exit 0 — and the fallback line names that reason rather than reading `narration ()`. No stack dump |
...
| #   | What                                   | Expected                                                                                                          | Result |
| --- | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ------ |
| 69  | `/merchant/<handle>/settings` in dev   | Editable profile; handle + category locked, each with its own reason                                              | **PASS** (owner-reported) — profile editable, handle and category locked with their own reasons |
...
| 85  | A shop whose payout is a CONTRACT ACCOUNT edits its profile | The slow tier settles it: ECDSA recovery cannot attribute the signature to the payout, so only `verifyTypedData`'s universal validator can accept it | **PASS (21 Aug).** `Erc1271Owner` (`src/mocks/`) deployed to `0x05669822…d7bD` with Anvil #0 as its owner, shop `erc1271-test-shop` registered with that CONTRACT as payout, then edited with an owner-signed proof: **200**. First execution of the slow tier — every other row in this phase used an EOA payout. **The deployed instance predates the constructor's zero-owner guard** (added after slither flagged it): that guard is constructor-only and this instance has a non-zero owner, so it cannot affect what was tested. Not redeployed because it could not be swapped in anyway — the shop's payout IS that contract, and `setMerchantPayout` is gated on `msg.sender == merchant.payout`, which a contract with no execution path cannot satisfy |
