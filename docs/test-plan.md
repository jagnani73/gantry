...
| #   | What                                                   | Expected                                                                                                                          | Result |
| --- | ------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | ------ |
| 26  | `/merchant/ah-hock-chicken-rice/qr` standee copy       | Must NOT claim it is the x402 door — it names a shop and never a price                                                            | —      |
| 27  | Type `1.5` in Charge an amount                         | Prose reads **S$1.50**                                                                                                            | —      |
| 28  | Type `50000`                                           | **S$50000.00** — no thousands separator, deliberately                                                                             | —      |
| 29  | Type `1.234` / `abc`                                   | Shape error, no QR                                                                                                                | —      |
| 30  | Copy the link                                          | Points at the **backend** origin, not the payer app                                                                               | —      |
| 31  | Open it in a browser                                   | Payer page with the pad **prefilled**                                                                                             | —      |
| 32  | `curl -i -H 'Accept: application/json' "<link>"`       | **402** with `PAYMENT-REQUIRED`                                                                                                   | —      |
| 33  | `curl -i "<link>"` (bare `*/*`, what curl sends)       | **402**, not a redirect. `req.accepts()` would answer html here and silently close the machine door with every test still passing | —      |
| 34  | `curl -i -H 'X-Forwarded-Host: evil.example' "<link>"` | Must NOT redirect to evil.example                                                                                                 | —      |
| 35  | `?sgd=1&sgd=2`                                         | Pad starts empty — two prices in a link is a broken link                                                                          | —      |
...
| #   | What                                                                                                                                                           | Expected                                                                                                            | Result |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | ------ |
| 45  | `/app/agents`                                                                                                                                                  | Each agent's caps named in **its own token**                                                                        | —      |
| 46  | With a USDC and a EURC agent both present                                                                                                                      | Copy reads "each agent's own token" rather than naming one                                                          | —      |
| 47  | Open an agent                                                                                                                                                  | **"Turned away"** beside "Allowed at", with the line saying it can only account for the categories this build knows | —      |
| 48  | The line under that card                                                                                                                                       | States a category is what KIND of shop, never WHICH one                                                             | —      |
| 49  | Change name + categories together                                                                                                                              | Three transactions in order; toast reads "All changes saved on-chain."                                              | —      |
| 50  | While that read catches up                                                                                                                                     | List row shows **"Saving…"**, never the new categories beside the old name                                          | —      |
...
- **The freshness gate on the agents list.** See **F4**.
