...
- Live-LLM agent mode needs `GOOGLE_GENERATIVE_AI_API_KEY` in `apps/agent/.env` (AI Studio free tier; unset ⇒ scripted mode, same beats); `AGENT_SESSION_KEY` lives there too — its address is the wallet's on-chain `agentSigner`. Demo prompts must carry the amount ("buy the team lunch (S$19.50) from Ah Hock") — an amount-less prompt lets the live model pick its own basket (it once paid its full remaining S$30.50 budget).
